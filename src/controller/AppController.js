...
const signup = [
    async (req, res) => {
        ...
        try {
            const { fname, lname, gender,phone, location, email,password } = req.body

            if (!email || !fname || !lname || !gender ||!phone|| !location || !password) {
...
const getReportedDisease = [
    requireAuth, async(req, res)=>{
        try {
            ...
            if(userType === "farmer"){
                return res.json({ error: { status: 401, message: 'please you are not allowed to perform this action' } })
            }
...
const approveReport = [
    requireAuth, async(req, res)=>{
        try {
            ...
            if(!report){
                return res.json({ error: { status: 404, message: 'there is no report found for this disease id' } });
            }
...
const deleteUser = [
    requireAuth, async(req, res)=>{
        try {
            ...
            if(userType !== 'rab'){
                throw new Error(req.user.fname + ' ' + 'you are not allowed to perform this action');
            }
            ...
            if(result.deletedCount !== 1){
                throw new Error("user not found for this phone number", req.params.phone)
            }
